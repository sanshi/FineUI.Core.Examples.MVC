...
{
    /// <summary>
    /// 含文件上传的示例控制器基类：在 <see cref="BaseController"/> 之上补「安全上传」便捷方法
    /// （校验、保存到 wwwroot 之外、生成公共下载地址）。
    /// 具体实现都在 <see cref="UploadStorage"/> 里，本类只是控制器侧的门面。
    /// </summary>
    public class BaseUploadController : BaseController
    {
        ...
        /// 注意：显式指定 area = ""，避免在 Area（如 Form）内调用时生成带 area 前缀的错误地址（如 /Form/Home/Download）。
        /// </summary>
