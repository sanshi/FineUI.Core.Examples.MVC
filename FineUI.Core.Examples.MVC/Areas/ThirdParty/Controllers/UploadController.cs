...
namespace FineUI.Core.Examples.MVC.Areas.ThirdParty.Controllers
{
    ...
    /// 读回走公共下载入口 /Home/Download（下载链接在页面脚本里按行拼，不带 inline），
    /// 强制 application/octet-stream + Content-Disposition: attachment，
    ...
    public class UploadController : FineUI.Core.Examples.MVC.Controllers.BaseUploadController
    {
        ...
        // 单个上传文件的大小上限，与页面上 WebUploader 的 fileSingleSizeLimit 保持一致。
        // 不设的话 Kestrel 用默认的 30MB，示例页写着 100MB 却在 30MB 处以 413 失败。
        private const long MAX_UPLOAD_SIZE = 100L * 1024 * 1024;

        [HttpPost]
        [ValidateAntiForgeryToken]
        [RequestSizeLimit(MAX_UPLOAD_SIZE)]
        [RequestFormLimits(MultipartBodyLengthLimit = MAX_UPLOAD_SIZE)]
        public IActionResult Process(IFormFile file, string owner)
        {
