using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Web;
using Microsoft.AspNetCore.Mvc;

namespace FineUI.Core.Examples.MVC.Areas.GridPaging.Controllers
{
    [Area("GridPaging")]
    public class DatabaseUpdatePageIndexController : FineUI.Core.Examples.MVC.Controllers.BaseController
    {
        // GET: GridPaging/Database
        public IActionResult Index()
        {
            LoadData();

            return View();
        }

        #region BindGrid

        private void LoadData()
        {
            var recordCount = DataSourceUtil.GetTotalCount();

            // 1.设置总项数（特别注意：数据库分页初始化时，一定要设置总记录数RecordCount）
            ViewBag.Grid1RecordCount = recordCount;

            // 2.获取当前分页数据
            ViewBag.Grid1DataSource = DataSourceUtil.GetPagedDataTable(pageIndex: 0, pageSize: 5);

        }

        #endregion

        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Grid1_PageIndexChanged(string[] Grid1_fields, int Grid1_pageIndex)
        {
            var grid1 = UIHelper.Grid("Grid1");

            var recordCount = DataSourceUtil.GetTotalCount();

            // 1.设置总项数（数据库分页回发时，如果总记录数不变，可以不设置RecordCount）
            grid1.RecordCount(recordCount);

            // 2.获取当前分页数据
            var dataSource = DataSourceUtil.GetPagedDataTable(pageIndex: Grid1_pageIndex, pageSize: 5);
            grid1.DataSource(dataSource, Grid1_fields);

            return UIHelper.Result();
        }


        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult btnUpdatePageIndex_Click(string[] Grid1_fields)
        {
            //var recordCount = DataSourceUtil.GetTotalCount();
            //// 1.设置总项数（数据库分页回发时，如果总记录数不变，可以不设置RecordCount）
            //grid1.RecordCount(recordCount);

            //// 2. 设置当前分页索引（最后一页）
            //// 注意：需要先设置 RecordCount，再设置 PageIndex，因为设置 PageIndex 时会验证分页索引的有效性！
            //var lastPageIndex = DataSourceUtil.GetPageCount(pageSize: 5) - 1;
            //grid1.PageIndex(lastPageIndex);


            var grid1 = UIHelper.Grid("Grid1");

            var recordCount = DataSourceUtil.GetTotalCount();
            var lastPageIndex = DataSourceUtil.GetPageCount(pageSize: 5) - 1;

            // 1.设置总项数和分页索引
            grid1.PageOptions(recordCount: recordCount, pageIndex: lastPageIndex);

            // 2.获取当前分页数据
            var dataSource = DataSourceUtil.GetPagedDataTable(pageIndex: lastPageIndex, pageSize: 5);
            grid1.DataSource(dataSource, Grid1_fields);

            return UIHelper.Result();
        }


    }
}